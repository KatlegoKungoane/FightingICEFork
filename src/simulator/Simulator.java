...
public class Simulator {
     ...
     * @param playerNumber
     *                        boolean value which identifies P1/P2. {@code true} if
     *                        the
     *                        player is P1, or {@code false} if P2.
     * @param myAct
     *                        actions to be performed by the player identified by
     *                        the
     *                        parameter `player`
     * @param oppAct
     ...
     *                        will
     *                        simulate the player's all actions and then continue
     *                        the
     *                        simulation until the `simulationLimit`-th frame
     *                        assuming no
    ...
    public FrameData simulate(FrameData frameData, boolean playerNumber, Deque<Action> myAct, Deque<Action> oppAct,
        ...
        return this.simulate(
                ...
                oppAct,
                simulationLimit,
                this.gameData.getMotion(true),
                this.gameData.getMotion(false));
    }

    private FrameData simulate(
            FrameData frameData,
            boolean playerNumber,
            Deque<Action> myAct,
            Deque<Action> oppAct,
            int simulationLimit,
            ArrayList<Motion> p1MotionData,
            ArrayList<Motion> p2MotionData) {
        ...
        ArrayList<ArrayList<Motion>> tempMotionList = new ArrayList<>(Arrays.asList(p1MotionData, p2MotionData));
